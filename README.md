...
1. **Ingestion:** Documents from "knowledge_files" folder are embedded and stored in a vector database.
...
2. Download llm model llama3.2 and embedding model mxbai-embed-large by running <br /> ```ollama pull llama3.2; ollama pull mxbai-embed-large```
3. Install python package manager uv with command```pip install uv```
4. Clone this repo in a folder.
5. Run in cmd ```uv tool install . -e```. <br /> This will automatically install all depedencies and create an executable mind-palace.exe.
6. Run in cmd  ```mind-palace``` to start application

A dummy source file has already been added to ./knowledge_files
so you can just ask about the info present in the file to the agent.

To add more files, add them to ./knowledge_files

## Sample Conversation
![Sample Conversation](docs/Sample_conversation.png)
