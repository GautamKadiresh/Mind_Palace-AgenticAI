...
def router(state: State):
    ...
    if user_query== "q":
        ...
        return {"messages": state["messages"], "user_query" : user_query, "retrieved_info" : None , "next_action" : 'retrieve'}
...
def retriever_agent(state: State):    
    ...
    return {"messages": state["messages"], "user_query" : state["user_query"], "retrieved_info" : retrieved_info, "next_action" : 'chat'}

def assistant_agent(state: State):
    chat_history = llm_assistant(messages=state["messages"],user_query=state["user_query"], retrieved_info=state["retrieved_info"])
    print('Mind Palace:')
